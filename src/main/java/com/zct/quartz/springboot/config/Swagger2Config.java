 ...
 */
package com.zct.quartz.springboot.config;
...
public class Swagger2Config {
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()) //创建API基本信息
                ...
                .select() //控制要暴露的接口
                .apis(RequestHandlerSelectors.basePackage("com.zct.quartz.springboot.controller")) //通过指定扫描包暴露接口
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("job RESTFUL APIs").description("业务服务").termsOfServiceUrl("")
                .contact(new Contact("zct", "https://github.com/OneYoungWorld/job_plus.git", "")).version("1.0").license("").licenseUrl("").build();
    }
}
